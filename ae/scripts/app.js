...
//run block
runBlock.$inject = ['$rootScope', '$window', '$http'];  //, '$q'
function runBlock($rootScope, $window, $http) {     // $q,
  ...
  function isUae() {
    ...
    $http.get('https://freegeoip.net/json/').then(function(response){
      ...
      if (response.data.country_code === ("AE" || "QA" || "BH" || "OM" || "SA" || "KW") ){
        VRinUAE = true;
      }
    },function(error){
